...
export function createFixtureForFiltrationTest(
  ...
  const parentOfSecondPage =
    pagesFromData[secondPagePageOfSecondLevel.parentId as TableOfContentsPageId];
  ...
  const topLevelPagesIds = Array.from(
    new Set<TableOfContentsPageId>([
      parentOfFirstPage.id,
      parentOfSecondPage.id,
      unmatchedTopLevelPage.id,
    ]),
  );
