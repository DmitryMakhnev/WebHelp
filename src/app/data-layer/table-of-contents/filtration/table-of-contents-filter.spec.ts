...
describe('TableOfContentsFilter', () => {
  ...
  it('some level page was found', () => {
    ...
    expect(
      (filtrationResult.tableOfContent as TableOfContentsApiResponse)
        .entities.pages[thirdLevelPage.id],
    ).toBe(thirdLevelPage);
  });
  ...
  it('filtration result contains parents pages of matched page', () => {
    ...
    const thirdLevelPage = Object.values(pagesFromData)
      .find(
        page => page.level === 2
          && page.parentId
          && pagesFromData[page.parentId].title !== page.title,
      );
    if (!thirdLevelPage || !thirdLevelPage.parentId) {
      throw new IncorrectFixtureError('Don\'t have any page on third level with parent with different title');
    }
    const filter = createTableOfContentsFilter(response);
    filter.filterByText(thirdLevelPage.title);
    const filtrationResultPages = (filter.filtrationResult
      .tableOfContent as TableOfContentsApiResponse)
      .entities.pages;
