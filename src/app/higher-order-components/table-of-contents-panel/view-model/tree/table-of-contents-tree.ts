...
import {
  createTableOfContentsTreeNode,
  TableOfContentsTreeNode,
} from './table-of-contents-tree-node';
import {
  createTableOfContentsTreeAnchors,
  TableOfContentsTreeAnchors,
} from './table-of-contents-tree-anchors';
...
export class TableOfContentsTree {
  constructor(private readonly tableOfContents: TableOfContentsApiResponse | null) {
  ...
  private readonly defaultIndexOfBuildNodes = new Map<
    TableOfContentsPageId,
    TableOfContentsTreeNode
  >();

  // eslint-disable-next-line max-len
  private filtrationModeIndexOfBuildNodes = new Map<
    TableOfContentsPageId,
    TableOfContentsTreeNode
  >();
  ...
  get currentAnchors(): TableOfContentsTreeAnchors {
    ...
    const anchors =
      this.tableOfContents && pageId ? findAnchorsOfPage(this.tableOfContents, pageId) : [];
  ...
  manageNodeContent(node: TableOfContentsTreeNode, isContendBuilt: boolean) {
    if (node.isHasChildPages && node.isContendBuilt !== isContendBuilt) {
      ...
      if (isContendBuilt) {
        children = this.createNodes(node.page.pages as TableOfContentsPageId[]);
  ...
  @action
  selectByPageId(pageId: TableOfContentsPageId, buildNodeContent: boolean = false): boolean {
    ...
    if (isPageWasSelected) {
      ...
      if (!node && this.tableOfContents) {
        const pathToNode = getPagesPathToPageFromRoot(this.tableOfContents, pageId);
        if (pathToNode) {
      ...
      if (buildNodeContent) {
        ...
        this.manageNodeContent(resultSelectedNode, true);
      }
    }
  ...
  private buildNodesByPath(pathToNode: TableOfContentsPageId[]) {
  ...
  private buildAllNodeOfNodes(nodes: TableOfContentsTreeNode[]) {
    const nodesForContentCreation: TableOfContentsTreeNode[][] = [nodes];

    for (let i = 0; i !== nodesForContentCreation.length; i += 1) {
      const currentNodes = nodesForContentCreation[i] as TableOfContentsTreeNode[];
      const iMax = currentNodes.length;
      for (let j = 0; j !== iMax; j += 1) {
        const node = currentNodes[j];
        if (node.isHasChildPages) {
          this.manageNodeContent(node, true);
          nodesForContentCreation.push(node.children as TableOfContentsTreeNode[]);
        }
      }
    }
  }

  private createNodes(pagesIds: TableOfContentsPageId[]): TableOfContentsTreeNode[] {
    const pages = (this.currentTableOfContents as TableOfContentsApiResponse).entities.pages;
    return pagesIds.map(page => createTableOfContentsTreeNode(this, pages[page]));
  }
  ...
  private unregisterNodesInIndex(nodes: TableOfContentsTreeNode[]) {
    ...
    while (allNodesForUnregistering.length) {
      ...
      if (currentNode.isContendBuilt) {
        allNodesForUnregistering = allNodesForUnregistering.concat(
          currentNode.children as TableOfContentsTreeNode[],
        );
      }
    }
  }
