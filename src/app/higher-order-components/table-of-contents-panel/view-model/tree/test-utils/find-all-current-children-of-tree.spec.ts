...
describe('findAllCurrentChildrenOfTree', () => {
  it('works correct', () => {
    ...
    const firstLevelPageWithContent = Object.values(response.entities.pages).find(
    ...
    const builtNodePageIds = buildNodes
      .map(node => node.page.id)
      .reduce((nodesIds, nodeId) => nodesIds.add(nodeId), new Set<TableOfContentsPageId>());
    const idsOfPagesMustBeBuilt = response.topLevelIds
      .concat(firstLevelPageWithContent.pages as TableOfContentsPageId[])
      .reduce((pageIds, pageId) => pageIds.add(pageId), new Set<TableOfContentsPageId>());
